...
class ContextualLinearEnvironment(LinearEnvironment):
    ...
    def __init__(self, n_rounds, contexts, arms, theta, psi=None, noise_std=0.01, random_state=1):
    ...
    def round(self, arm_i):
        obs_reward = (self.theta @ self.psi(self.arms[arm_i], self.contexts[self.last_context_i])
                      + self.noise[self.t])
        self.rewards = np.append(self.rewards, obs_reward)
        self.t += 1
